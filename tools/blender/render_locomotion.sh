...
OUTPUT="${LEO_LOCOMOTION_OUTPUT:-$ROOT/assets/renders/work/locomotion}"
...
"$BLENDER" --background "$SOURCE" \
  --python "$ROOT/tools/blender/render_locomotion.py" -- \
  --output-dir "$OUTPUT"
"$PYTHON" "$ROOT/tools/blender/canonicalize_renders.py" --render-dir "$OUTPUT"
"$PYTHON" "$ROOT/tools/blender/make_locomotion_qa.py" --render-dir "$OUTPUT"

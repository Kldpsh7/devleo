...
OUTPUT="${LEO_GESTURE_OUTPUT:-$ROOT/assets/renders/work/gestures}"
...
"$BLENDER" --background "$SOURCE" \
  --python "$ROOT/tools/blender/render_gestures.py" -- \
  --output-dir "$OUTPUT"
"$PYTHON" "$ROOT/tools/blender/canonicalize_renders.py" --render-dir "$OUTPUT"
"$PYTHON" "$ROOT/tools/blender/make_gesture_qa.py" --render-dir "$OUTPUT"
